...
module.exports = {
    ...
    run: async (_, interaction) => {
        ...
        selfEmbed.setDescription(
            `**ლინკები:**\n[png](${targetUser.user.displayAvatarURL({
                dynamic: false,
                size: 1024,
                format: "png",
            })}) | [jpg](${targetUser.user.displayAvatarURL({
                dynamic: false,
                size: 1024,
                format: "jpg",
            })}) | [webp](${targetUser.user.displayAvatarURL({
                dynamic: true,
                size: 1024,
                format: "webp",
            })})`
        ...
        selfEmbed.setImage(
            `${targetUser.user.displayAvatarURL({ dynamic: true, size: 1024 })}`
        );
